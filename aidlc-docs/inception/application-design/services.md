...
>
> **Pre-GSP-010 consistency pass (D-07).** `RdfService.applyPatch` and `PatchService.scopeToGraph`/`apply` widened from `targetIri: string` to `targetIri: string | null`, catching these signatures up to GSP-010 v4's default-graph PATCH support (`null` = default graph, matching `parseWithReconciliation`/`triplesToDataset`). §6 removed a stale duplicate `merge` definition left over from an earlier draft and renumbered §6.4–6.9 to §6.4–6.8 accordingly. Added an explicit scope note that `serializeStream` (present in `components.md`'s method table) is SHOULD-level per NFR-04 and not required for GSP-010 v1 sign-off.
...
> **Scope note — `serializeStream`.** `components.md` §2.6 lists `serializeStream(dataset, mediaType): Readable` as a public method. NFR-04 (streaming for large payloads) is a **SHOULD**, not a MUST, so `serializeStream` is **not required for GSP-010 v1 sign-off**; `serialize`/`serializeToDataset` (buffered) satisfy every MUST-level acceptance criterion. Implement `serializeStream` in GSP-010 only if time permits, or defer it to UW-07 (Observability/NFR hardening) — either is acceptable, but pick one explicitly rather than leaving it an implicit gap between the two documents.

---

### 6.4 Method: `triplesToDataset`
...
### 6.7 Method: `parseWithReconciliation`
...
**Purpose:** Apply a pre-parsed SPARQL Update AST to existing rows. `targetIri: null` addresses the default graph (GSP-010 v4), matching the convention already used by `parseWithReconciliation`/`triplesToDataset`.
...
**Behavior:** Inspects parsed AST for references to any graph other than `targetIri`. `targetIri: null` (default graph) additionally rejects any `GRAPH` clause outright, since the default graph has no IRI to match against. Multi-graph, or any `GRAPH` clause when `targetIri` is `null` → `{ valid: false }` (→ 422).
